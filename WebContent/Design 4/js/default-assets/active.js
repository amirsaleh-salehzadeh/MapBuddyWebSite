(function ($) {
    ...
    razo_window.on('scroll', function () {
        if (razo_window.scrollTop() > 0) {
        	...
        	$('#umpLogoImg').addClass('sticky');
        	$('.logoTextSVG').addClass('sticky');
        	$('.nav-brand').addClass('sticky');
        } else {
            $('.main-header-area').removeClass('sticky');
            $('.logoTextSVG').removeClass('sticky');
            $('#umpLogoImg').removeClass('sticky');
        	$('.nav-brand').removeClass('sticky');
        }
    });
...
jQuery('img.svgMenu').each(function(){
    var $img = jQuery(this);
    var imgID = $img.attr('id');
    var imgClass = $img.attr('class');
    var imgURL = $img.attr('src');
